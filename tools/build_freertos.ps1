...
if (-not (Test-Path $ConfigFile)) {
    throw "Copie tools/local_config.example.ps1 como tools/local_config.ps1 y edite las rutas."
}
. $ConfigFile
...
foreach ($VariableName in @("GD32_MSDK_ROOT", "NUCLEI_TOOLCHAIN_DIR", "OPENOCD_ROOT")) {
    ...
    if ([string]::IsNullOrWhiteSpace($Value)) {
        throw "$VariableName no esta configurada en tools/local_config.ps1."
    }
}
...
if (-not (Test-Path $ImagePath)) {
    throw "No se genero image-all.bin: $ImagePath"
}
